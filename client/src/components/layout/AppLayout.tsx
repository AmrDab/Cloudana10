...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const totalSpent = jobs.reduce((sum, job) => sum + parseFloat(formatEther(job.spent ?? BigInt(0))), 0);
  ...
  // Keep mobile menu behavior predictable while navigating and prevent body scroll when open.
  useEffect(() => {
  ...
  const navItems: NavItem[] = [
    ...
    { label: "Mining", path: "/mining", icon: Pickaxe, show: true },
    { label: "Docs", path: "/docs", icon: BookOpen, show: true },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex font-sans selection:bg-primary/20 selection:text-primary">
      ...
      <div className="flex-1 flex flex-col min-w-0 md:pl-[14rem]">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden fixed inset-x-0 top-16 bottom-0 z-40 overflow-y-auto bg-background/95 backdrop-blur-xl border-b border-white/10 animate-in slide-in-from-top-5">
        ...
        <main className="flex-1 container mx-auto w-full px-4 py-6 md:py-8">{children}</main>
        ...
        <footer className="border-t border-white/5 bg-black/20 py-8 mt-auto">
          <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-6">
              ...
              <a href="https://sepolia.basescan.org/" target="_blank" rel="noreferrer" className="hover:text-primary transition-colors">
                Explorer
              </a>
              <a href="https://status.base.org/" target="_blank" rel="noreferrer" className="hover:text-primary transition-colors">Status
              </a>
            </div>
          </div>
        </footer>
