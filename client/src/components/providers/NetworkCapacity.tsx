    ...
    >
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-xs">
          {Math.round(activeCPU)} / {Math.round(totalCPU)} cores
        </p>
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-xs">
          {totalGPU === 0 ? "No GPU" : `${Math.round(activeGPU)} / ${Math.round(totalGPU)} GPUs`}
        </p>
        {totalGPU > 0 && <Progress value={gpuPct} className="h-2" />}
      </div>
      <div className="space-y-2">
        <p className="text-sm font-semibold tracking-tight">RAM</p>
        <p className="text-muted-foreground text-xs">
          {roundDecimal(am.value, am.unit === "GB" || am.unit === "TB" ? 1 : 2)} {am.unit} / {roundDecimal(tm.value, tm.unit === "GB" || tm.unit === "TB" ? 1 : 2)} {tm.unit}
        </p>
        <Progress value={memPct} className="h-2" />
      </div>
      <div className="space-y-2">
        <p className="text-sm font-semibold tracking-tight">Disk</p>
        <p className="text-muted-foreground text-xs">
          {roundDecimal(as.value, as.unit === "GB" || as.unit === "TB" ? 1 : 2)} {as.unit} / {roundDecimal(ts.value, ts.unit === "GB" || ts.unit === "TB" ? 1 : 2)} {ts.unit}
        </p>
