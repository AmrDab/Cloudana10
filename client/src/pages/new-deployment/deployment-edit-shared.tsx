...
export function deployToBuilderConfig(parsed: Record<string, unknown>): Partial<BuilderConfig> {
  ...
  if (!parsed?.services || typeof parsed.services !== 'object') {
    console.error('[Builder Config] ERROR: Missing or invalid services section');
    console.error('[Builder Config] Available keys:', Object.keys(parsed || {}));
    return {
      image: "nginx:latest",
      cpu: 1,
      memory: 1,
  ...
  if (serviceEntries.length === 0) {
    console.error('[Builder Config] ERROR: No services defined');
    return {
      image: "nginx:latest",
      cpu: 1,
      memory: 1,
  ...
  const env = envArr.map((e) => {
    if (typeof e === 'string') {
      ...
      // Handle object format { key: "KEY", value: "value" }
      return { key: (e as any).key || "", value: (e as any).value || "" };
    }
    return { key: "", value: "" };
  });
  ...
  if (!parsed?.profiles || typeof parsed.profiles !== 'object') {
    console.warn('[Builder Config] WARNING: Missing profiles section, using defaults');
  }
  ...
  if (!compute) {
    console.warn('[Builder Config] WARNING: No compute profile found, using defaults');
  }
  ...
  const cpuRaw = resources.cpu as { units?: number } | number | undefined;
  let cpu: number;
  ...
  const memRaw = resources.memory as { size?: string } | string | undefined;
  let memSize: string;
  ...
  let stor: Array<{ size?: string; name?: string; mount?: string; attributes?: { persistent?: boolean; class?: string } }>;
...
export function extractDeploySummary(config: string): DeploySummary | null {
  try {
    ...
    console.log('[Deploy Summary] Builder config:', {
      ...
      hasExpose: b.expose?.length > 0,
      hasEnv: b.env?.length > 0,
    });
    
    // Ensure we have required fields
    if (!b.image) {
      console.error('[Deploy Summary] ERROR: Missing image field');
      return null;
    }
    
    if (typeof b.cpu !== 'number') {
      console.error('[Deploy Summary] ERROR: Invalid CPU value:', b.cpu);
      return null;
    }
    
    if (typeof b.memory !== 'number' || !b.memoryUnit) {
      console.error('[Deploy Summary] ERROR: Invalid memory values:', { memory: b.memory, unit: b.memoryUnit });
      return null;
    }
    ...
    console.error('[Deploy Summary] Error details:', e instanceof Error ? e.message : String(e));
    if (e instanceof Error && e.stack) {
      console.error('[Deploy Summary] Stack:', e.stack.split('\n').slice(0, 3).join('\n'));
    }
    return null;
  }
}
