...
// Wallet (optional — not needed for provider env install; orchestrator handles rewards)
...
// Build Provider Request Schema (wallet omitted — provider node does not need key for execution/reporting)
...
export const BuildProviderStatusResponseSchema = z.object({
  ...
  /** Unique device id from installed provider node (for on-chain registration). Set after install_provider_node_server. */
  device_id: z.string().nullable().optional(),
  /** Real hardware spec from node (GET /device-info). Used to cap offered spec in registration. */
  device_spec: z.object({
    cpuModel: z.string(),
    cpuCores: z.number(),
  ...
  /** Control node endpoint (provider node URL) for orchestrator to send workloads. Format: http://IP:4040 */
  control_node_endpoint: z.string().nullable().optional(),
});
