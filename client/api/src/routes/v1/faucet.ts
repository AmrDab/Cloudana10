...
const FAUCET_FILE = path.resolve(DATA_DIR, "faucet-claims.json");

let claims: Record<string, number> = {}; // address → last claim timestamp

function loadClaims() {
  try {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
    if (fs.existsSync(FAUCET_FILE)) {
      claims = JSON.parse(fs.readFileSync(FAUCET_FILE, "utf-8"));
    }
  } catch {
    claims = {};
  }
...
function saveClaims() {
  try {
    if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
    fs.writeFileSync(FAUCET_FILE, JSON.stringify(claims, null, 2));
  } catch (err) {
    L.error("Failed to save faucet claims", err);
  }
}
...
function loadAbi(name: string): unknown[] {
  const sharedAbiPath = process.env.SHARED_PATH
    ...
    : path.join(process.cwd(), "..", "..", "shared", "abi");
  const file = path.join(sharedAbiPath, `${name}.json`);
  if (!fs.existsSync(file)) return [];
  return JSON.parse(fs.readFileSync(file, "utf8"));
}

const CLDTokenAbi = loadAbi("CLDToken");
...
faucetRouter.post("/faucet/claim", async (c) => {
  try {
    ...
    if (receipt.status === "success") {
      claims[address] = now;
      saveClaims();
      L.success(`🚰 Faucet: Minted 100 CLD to ${address} ✅ (tx: ${hash})`);
      return c.json({
        success: true,
        txHash: hash,
        amount: "100",
