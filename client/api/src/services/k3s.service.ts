...
export interface IK3sSSHAdapter {
  ...
  /** Optional: upload a buffer to remote path (for provider-node-server tarball). */
  putFile?(client: SSHClient, remotePath: string, buffer: Buffer): Promise<void>;
}
...
export class K3sService {
  ...
  static readonly FALLBACK_IP_CMD = `hostname -I | awk '{print $1}'`;
  ...
  /**
   * Detect node IP with fallback for public-IP-only nodes.
   * First tries to find private IP, then falls back to first available IP or external IP.
   */
  private async detectNodeIp(
    ...
    let ip = internalIpOut.trim();
    
    // Fallback to first available IP if no private IP found
    if (!ip) {
      log("warn", "No private IP detected, using fallback IP detection");
      const { stdout: fallbackIpOut } = await this.adapter.runCommand(
        client,
        K3sService.FALLBACK_IP_CMD,
        opts()
      );
      ip = fallbackIpOut.trim();
    }
    
    // Ultimate fallback to external IP if provided
    if (!ip && externalIp) {
      log("warn", `No IP detected via commands, using external IP: ${externalIp}`);
      ip = externalIp;
    }
    ...
    if (!ip) {
      throw new ApplicationError({
        statusCode: HTTP_500,
        errorCode: "K3S_IP_DETECT_FAILED",
        payload: {
          error: "IP Detection Failed",
          message: "Could not detect any IP address on the node. Please check network configuration.",
        },
      });
    }
    
    log("info", `Detected node IP: ${ip}`);
    return ip;
  }
  ...
  async initializeK3sControl(
    ...
    try {
      ...
      // Detect node IP with fallback for public-IP-only nodes
      const nodeIp = await this.detectNodeIp(sshClient, externalIp, taskId, onLog);

      // Use external IP for node-ip if available (better for public nodes), otherwise use detected IP
      const primaryIp = externalIp || nodeIp;
      installExec += ` --node-ip=${primaryIp} --advertise-address=${primaryIp} --kube-scheduler-arg=config=/var/lib/rancher/k3s/server/etc/scheduler-config.yaml`;
      log("info", `Setting node IP to ${primaryIp}`);
      ...
      // Add both detected IP and external IP to TLS SAN for flexibility
      installExec += ` --tls-san=${nodeIp}`;
      if (externalIp && externalIp !== nodeIp) {
        installExec += ` --tls-san=${externalIp}`;
      }
      log("info", `Adding IPs to TLS SAN: ${nodeIp}${externalIp && externalIp !== nodeIp ? ", " + externalIp : ""}`);
  ...
  async configureFirewall(sshClient: SSHClient, taskId: string, onLog?: (line: string) => void): Promise<void> {
    const opts = () => runOpts(taskId, onLog);
    try {
      log("info", "Configuring firewall for provider node and Kubernetes workloads");
      
      // Check if UFW is installed and active
      const { stdout: ufwStatus } = await this.adapter.runCommand(
        sshClient,
        "which ufw && ufw status | head -1 || echo 'not-installed'",
        runOpts(taskId, onLog, { checkExitStatus: false })
      );
      
      if (ufwStatus.includes("not-installed") || ufwStatus.includes("inactive")) {
        log("info", "UFW is not active, skipping firewall configuration");
        onLog?.("UFW firewall not active, skipping port configuration");
        return;
      }

      log("info", "UFW is active, configuring required ports");
      
      // Allow port 4040 for provider node API
      await this.adapter.runCommand(
        sshClient,
        "ufw allow 4040/tcp",
        opts()
      );
      onLog?.("✓ Opened port 4040 (provider node API)");
      
      // Allow NodePort range for Kubernetes workload services (30000-32767)
      await this.adapter.runCommand(
        sshClient,
        "ufw allow 30000:32767/tcp",
        opts()
      );
      onLog?.("✓ Opened ports 30000-32767 (Kubernetes NodePort range)");
      ...
      onLog?.("✓ Firewall configured - workloads can now be deployed");
    } catch (e) {
      log("error", `Error configuring firewall: ${String(e)}`);
      throw new ApplicationError({
        statusCode: HTTP_500,
        errorCode: "K3S_014",
        payload: {
          error: "Firewall Configuration Failed",
          message: `Error configuring firewall: ${String(e)}`,
        },
      });
    }
  }
  ...
  /** Writes ~/provider/provider.yaml. Key read from node only when wallet provided; otherwise placeholders (no wallet needed for provider env install). */
  async prepareProviderConfig(
    ...
    try {
      ...
      await sleep(2000);
      const useWallet = Boolean(opts.walletAddress?.trim());
      let keyContent = "";
      if (useWallet) {
        ...
        keyContent = "cGxhY2Vob2xkZXI="; // base64 placeholder
        log("info", "No wallet provided; using placeholder key (provider node does not need key for execution/reporting).");
      }
      const nodeUrl =
        opts.chainId === "akashnet-2" ? "http://akash-node-1:26657" : "https://rpc.sandbox-2.aksh.pw:443";
      const keySecretB64 = useWallet && opts.keyPassword
        ? Buffer.from(opts.keyPassword, "utf8").toString("base64")
        : "cGxhY2Vob2xkZXI=";
      const fromAddress = useWallet ? opts.walletAddress : "placeholder";
...
`;
  ...
  /**
   * Install Cloudana Provider Node server on the control node: Node.js (if needed), upload tarball,
   * extract, npm install, PM2, and start. Use pm2 logs cloudana-provider-node for easy log viewing.
   */
  async installProviderNodeServer(
    sshClient: SSHClient,
    taskId: string,
    onLog?: (line: string) => void,
    ...
    const opts = () => runOpts(taskId, onLog);
    try {
      log("info", "Installing Cloudana Provider Node server...");
      await sleep(2000);

      const hasNode = await this.checkCommandExists(sshClient, "node");
      if (!hasNode) {
        onLog?.("Node.js not found; installing via NodeSource...");
        await this.adapter.runCommand(
          sshClient,
      ...
      if (!this.adapter.putFile || !tarballBuffer || tarballBuffer.length === 0) {
        log("warn", "Provider node tarball not provided or adapter has no putFile; skipping install.");
        onLog?.("Skipping provider node server install (no tarball).");
        return;
      }

      const remoteTgz = "/tmp/cloudana-provider-node.tgz";
      await this.adapter.putFile(sshClient, remoteTgz, tarballBuffer);
      onLog?.("Tarball uploaded.");

      await this.adapter.runCommand(
        sshClient,
        "sudo mkdir -p /opt/cloudana-provider-node && sudo rm -rf /opt/cloudana-provider-node/*",
        opts()
      );
      await this.adapter.runCommand(
        sshClient,
        `sudo tar -xzf ${remoteTgz} -C /opt/cloudana-provider-node --strip-components=1`,
        opts()
      );
      await this.adapter.runCommand(sshClient, "sudo rm -f " + remoteTgz, opts());
      await this.adapter.runCommand(
        sshClient,
        "cd /opt/cloudana-provider-node && sudo npm install --omit=dev",
        opts()
      );

      onLog?.("Installing PM2...");
      await this.adapter.runCommand(sshClient, "sudo npm install -g pm2", opts());
      onLog?.("Starting Cloudana Provider Node with PM2...");
      await this.adapter.runCommand(
        sshClient,
        "cd /opt/cloudana-provider-node && sudo pm2 start ecosystem.config.cjs",
        opts()
      );
      await this.adapter.runCommand(sshClient, "sudo pm2 save", opts());
      onLog?.("Cloudana Provider Node started (pm2 logs cloudana-provider-node for logs).");
      log("info", "Cloudana Provider Node server installation completed.");
    } catch (e) {
      if (e instanceof ApplicationError) throw e;
      this.handleUnexpectedError(e, "Provider Node server installation");
    }
  }
  ...
  async updateKubeconfig(
    ...
    const opts = () => runOpts(taskId, onLog);
    try {
      log("info", "Updating kubeconfig file...");
      const kubeconfigPath = K3S_KUBECONFIG_PATH;

      // CRITICAL: Detect node IP with fallback - NEVER use empty IP in kubeconfig
      const serverIp = await this.detectNodeIp(sshClient, externalIp, taskId, onLog);
      ...
      if (!serverIp) {
        throw new ApplicationError({
          ...
          payload: {
            error: "IP Detection Failed",
            message: "Cannot create kubeconfig: no IP address detected on node",
          },
        });
      }

      log("info", `Using IP ${serverIp} for kubeconfig server URL`);
...
`;
  ...
  async joinControlNode(
    ...
    try {
      ...
      // Detect master node IP with fallback
      const masterIp = await this.detectNodeIp(controlSshClient, null, taskId, onLog);
      log("info", `Master node IP: ${masterIp}`);

      const workerClient = await this.adapter.connectToWorkerNode(controlSshClient, nodeInput);
      try {
        // Detect joining node IP with fallback (using hostname as external IP if available)
        const nodeIp = await this.detectNodeIp(workerClient, nodeInput.hostname || null, taskId, onLog);
        log("info", `Joining control node IP: ${nodeIp}`);

        let installExec = `--disable=traefik --flannel-backend=none --disable-network-policy --node-ip=${nodeIp} --node-name ${nodeName} --kube-scheduler-arg=config=/var/lib/rancher/k3s/server/etc/scheduler-config.yaml`;
  ...
  async joinWorkerNode(
    ...
    try {
      ...
      // Detect master node IP with fallback
      const masterIp = await this.detectNodeIp(controlSshClient, null, taskId, onLog);
      log("info", `Master node IP for worker join: ${masterIp}`);
  ...
  async updateCorednsConfig(sshClient: SSHClient, taskId: string, onLog?: (line: string) => void): Promise<void> {
    ...
    try {
      ...
      // Wait for CoreDNS ConfigMap with timeout (10 minutes)
      log("info", "Waiting for CoreDNS ConfigMap to be available...");
      const timeoutSeconds = 600; // 10 minutes
      const waitCommand = `
timeout=${timeoutSeconds}
elapsed=0
while ! kubectl -n kube-system get cm coredns >/dev/null 2>&1; do
  echo "waiting for the coredns configmap resource ... (elapsed: \${elapsed}s / ${timeoutSeconds}s)"
  if [ \$elapsed -ge \$timeout ]; then
    echo "ERROR: CoreDNS ConfigMap not found after \${timeout} seconds"
    echo "Checking cluster status:"
    kubectl get nodes 2>&1 || echo "kubectl get nodes failed"
    kubectl -n kube-system get pods 2>&1 || echo "kubectl get pods failed"
    kubectl -n kube-system get cm 2>&1 || echo "kubectl get configmaps failed"
    exit 1
  fi
  sleep 2
  elapsed=\$((elapsed + 2))
...
`;
      await this.adapter.runCommand(sshClient, waitCommand, opts());
      
      log("info", "Patching CoreDNS ConfigMap...");
  ...
  async createAndLabelNamespaces(sshClient: SSHClient, taskId: string, onLog?: (line: string) => void): Promise<void> {
    const opts = () => runOpts(taskId, onLog);
    log("info", "Creating and labeling Cloudana Kubernetes namespaces");
    try {
      const namespaces = ["cloudana-services", "workloads"];
      ...
      const labelCommands = [
        "kubectl label ns cloudana-services cloudana.network/name=cloudana-services cloudana.network=true --overwrite",
        "kubectl label ns workloads cloudana.network=true --overwrite",
      ];
