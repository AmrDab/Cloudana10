...
export class TemplateService {
  ...
  async getTemplateGallery(): Promise<TemplateCategory[]> {
    ...
    try {
      // Fetch templates from all three repositories in parallel
      ...
      const mergedCategories = mergeTemplateCategories(
      ...
      // Cache the results
      this.cachedTemplates = mergedCategories;
      ...
      if (this.cachedTemplates) {
        console.log("Returning cached templates due to error");
        return this.cachedTemplates;
      }
      throw error;
    }
  ...
  async getTemplateById(id: string): Promise<Template | null> {
    const categories = await this.getTemplateGallery();
