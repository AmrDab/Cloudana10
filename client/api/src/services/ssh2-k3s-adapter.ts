...
function putFile(conn: Client, remotePath: string, buffer: Buffer): Promise<void> {
...
export class Ssh2K3sAdapter implements IK3sSSHAdapter {
  ...
  async putFile(client: SSHClient, remotePath: string, buffer: Buffer): Promise<void> {
    return putFile(client as Client, remotePath, buffer);
  }
}
